...
impl NodeId {
    ...
    pub fn step(self, store: &mut Store) -> NodeId {
        ...
        let step = match self.base(store) {
            ...
            NodeBase::LevelFour { mut board } => {
                // can't step more than 2^(level - 2) = 4 times
                assert!(step_log_2 <= 2);
            ...
            NodeBase::Interior { nw, ne, sw, se } => {
                ...
                assert!(step_log_2 <= level - 2);

                if step_log_2 == level - 2 {
                    // step is equivalent to jump
                    self.jump(store)
                } else {
                    // normal recursion
...
mod tests {
    ...
    fn jump_glider() {
        ...
        // four steps
        let jump = glider.jump(&mut store);

        let expected = store.create_level_3(u8x8::new(
            0b0000_0000,
            0b0000_0000,
            0b0010_0000,
            0b0001_0000,
